...
`docker compose`로 인프라와 `central-server`를 함께 기동할 수 있다.
...
### Central Server만 로컬에서 직접 실행

애플리케이션만 IDE 또는 터미널에서 직접 실행하고 싶다면 인프라를 먼저 띄운 뒤 `bootRun`을 사용할 수 있다.
...
`application-local.yml`은 기본값으로 `localhost:13306`을 바라보고, Compose 안에서 실행될 때는 `DB_HOST=mysql-master`, `DB_PORT=3306` 환경변수를 사용한다.

### 모니터링 확인

Prometheus는 Compose 네트워크 안의 `central-server:8080`을 scrape target으로 사용한다.
